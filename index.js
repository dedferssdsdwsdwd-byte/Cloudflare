...
function parseAddrFromBuffer(buf, start = 0) {
    const u = buf instanceof Uint8Array ? buf : new Uint8Array(buf);
    let offset = start;
    if (u.length < offset + 1) throw new Error('short');
    const atyp = u[offset++];
    let address = '';
    if (atyp === 1) {
        if (u.length < offset + 4) throw new Error('short ipv4');
        address = Array.from(u.slice(offset, offset + 4)).join('.');
        offset += 4;
    } else if (atyp === 3) {
        const len = u[offset++];
        if (u.length < offset + len) throw new Error('short domain');
        address = new TextDecoder().decode(u.slice(offset, offset + len));
        offset += len;
    } else if (atyp === 4) {
        if (u.length < offset + 16) throw new Error('short ipv6');
        const parts = [];
        for (let i = 0; i < 8; i++) {
            parts.push(((u[offset + i * 2] << 8) | u[offset + i * 2 + 1]).toString(16));
        }
        address = parts.join(':');
        offset += 16;
    } else {
    ...
    if (u.length < offset + 2) throw new Error('short port');
    const port = (u[offset] << 8) | u[offset + 1];
    offset += 2;
    return { address, port, offset, bytesRead: offset - start };
}

async function handleUDPOverWS(ws, config, env, ctx) {
    try {
        const r = ws.readable.getReader();
        const w = ws.writable.getWriter();
        const first = await r.read();
        if (first.done) { try { w.releaseLock(); r.releaseLock(); } catch(e){} await safeCloseWebSocket(ws); return; }
        const initBuf = new Uint8Array(first.value);
        const hdr = await ProcessProtocolHeader(initBuf, env, ctx);
        if (hdr.hasError || !hdr.isUDP) { await safeCloseWebSocket(ws); return; }
        const leftover = initBuf.slice(hdr.rawDataIndex);
        if (leftover && leftover.length) {
            const p = leftover;
            try {
                const parsed = parseAddrFromBuffer(p, 0);
                const payload = p.slice(parsed.offset);
                if (parsed.port === 53) {
                    const resp = await fetch('https://1.1.1.1/dns-query', {
                        method: 'POST',
                ...
                console.error('UDP initial handling error:', err);
            }
        }
        while (true) {
            const { done, value } = await r.read();
            if (done) break;
            const p = new Uint8Array(value);
            try {
                const parsed = parseAddrFromBuffer(p, 0);
                const payload = p.slice(parsed.offset);
                if (parsed.port === 53) {
                ...
                console.error('UDP packet handling error:', err);
            }
        }
        try { r.releaseLock(); } catch (e) {}
        try { w.releaseLock(); } catch (e) {}
        await safeCloseWebSocket(ws);
    } catch (e) {
        console.error('handleUDPOverWS error:', e);
        await safeCloseWebSocket(ws);
    }
}

async function handleUpgradeRequest(request, config, env, ctx) {
    if (!request || !request.headers) return new Response('Bad Request', { status: 400 });
    if ((request.headers.get('upgrade') || '').toLowerCase() !== 'websocket') return new Response('Upgrade header required', { status: 400 });
    ...
    if (path === '/udp') {
        ...
        server.accept();
        handleUDPOverWS(server, config, env, ctx);
        return new Response(null, { status: 101, webSocket: client });
    }
